...
def pos_line(line, pos_vocab):
    return list(map(lambda word: to_token(pos_tag([word])[0][1], pos_vocab), line.lower().split()))

def translate(line, vocab, pos_vocab, model):
    input_tokens = line.lower().split()
    input_ids = np.array([[tokenize_line(line, vocab)]])
    input_pos = np.array([[pos_line(line, pos_vocab)]])
